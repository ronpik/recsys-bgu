...
def run_training(model_name: str, train_df: pd.DataFrame, num_latent_features: int) -> RecommenderSystem:
    ...
    elif model_name == SURPRISE_MODEL:
        cf_model.TrainSupriseModel(train_df)

    else:
        raise Exception(f"Not a valid model name to train: {model_name}")
...
def save_model(model_name: str, cf_model: RecommenderSystem, outpath: str):
    ...
    if model_name == SVD_MODEL:
        ...
        print(f"saving for model {model_name} is not currently available.")
